...
app.post("/login",upload.none(),async (req,res)=>{
  ...
  if(userDetails.length>0){
    ...
    if(isPassword == true){
      ...
      let token = jwt.sign({email:req.body.email,password:userDetails[0].password},process.env.SECRET_KEY);
...
app.post("/loginWithToken", upload.none(), async (req, res) => {
  ...
  try {
    let decryptedTokenObj = await jwt.verify(receivedToken, process.env.SECRET_KEY);
